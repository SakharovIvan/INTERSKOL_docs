...
Отправить `GET` запрос на `http://77.105.172.224:8000/api1c/order_pool`
...
`GET` на `http://77.105.172.224:8000/api1c/order_pool` с параметром даты `?date=MM-DD-YYYY`
...
`GET` на `http://77.105.172.224:8000/api1c/order_pool` с параметром даты `?GUID=example`

В ответ будет отправлен массив с одним заказом, соответсвующему данному GUID коду [формат ответа](#get_order_pool_answer)

_Пример: `http://77.105.172.224:8000/api1c/order_pool?GUID=12`_
...
`POST` на `http://77.105.172.224:8000/api1c/order_pool`
...
Отправьте `POST` запрос на `http://77.105.172.224:8000/api1c/order_pool`
в body запроса укажите массив в следующем формате
